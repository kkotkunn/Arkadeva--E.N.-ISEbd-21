...
{   /// <summary>
        ...
        where T : class, ITransport
    {
        ...
        /// <returns></returns>
        public static int operator +(Parking<T> p, T bbomber)
        {
            if (p._places.Count == p._maxCount)
            {
                throw new ParkingOverflowException();
            }
            if (p._places.ContainsValue(bbomber))
            {
            ...
            for (int i = 0; i < p._maxCount; i++)
            {
                ...
                {
                    p._places.Add(i, bbomber);
                    p._places[i].SetPosition(5 + i / 5 * _placeSizeWidth + 5,
                    i % 5 * _placeSizeHeight + 30,
        ...
        {
            DrawMarking(g);
            foreach (var bbomber in _places)
            {
                bbomber.Value.DrawBomber(g);
            }
        }
        /// <summary>
        ...
        public int CompareTo(Parking<T> other)
        {
            ...
            else if (_places.Count > 0)
            {
                ...
                {
                    if (_places[thisKeys[i]] is Airplane && other._places[thisKeys[i]] is Flybomber)
                    {
                        return 1;
                    }
                    if (_places[thisKeys[i]] is Flybomber && other._places[thisKeys[i]] is Airplane)
                    {
                        return -1;
                    }
                    ...
                    if (_places[thisKeys[i]] is Flybomber && other._places[thisKeys[i]] is Flybomber)
                    {
                        return (_places[thisKeys[i]] is Flybomber).CompareTo(other._places[thisKeys[i]] is Flybomber);
                    }
                }
            }
            return 0;
